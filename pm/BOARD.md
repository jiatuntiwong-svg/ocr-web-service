...
| UI-7 | Batch flow ในสไตล์ v2 (แทน stopgap ที่เปิด UI เดิม) | frontend-ui | 🟡 P1 | 👀 review — v2-native batch view landed (`src/components/OCRBatchViewV2.tsx` new, `OCRWorkspaceV2.tsx` ตัด v1 embed ออก), i18n th/en ครบ 52 keys/locale, tsc + build clean, v1 `OCRWorkspace.tsx` byte-identical (md5 verified). D1–D8 ทำครบ, Q1 answer = match v1 export (exportOCRBatch reuse), Q2/Q3 → defer UI-7b. Report: `pm/reports/UI-7.md`. + UAT r1 landed (F1-F5) — see `pm/reports/UI-7-uat-r1.md`. + UAT r2 (vertical file list + preview) — see `pm/reports/UI-7-uat-r2.md`. + UAT r3 (per-file mini preview in list) — see `pm/reports/UI-7-uat-r3.md`. + UAT r4 (full preview modal) — click any mini/page thumb opens full-size modal with ←/→ chevrons + Esc close + "รวมหน้านี้" toggle; see `pm/reports/UI-7-uat-r4.md`. + UAT r5 (P0 single-file fixes: fulldoc Object bug, Excel preview, Quick toggle, highlight fix) — 4 P0 items landed, tsc + build clean, v1 md5 unchanged, highlight #6 has partial fix + red-flag server-side follow-up; see `pm/reports/UI-7-uat-r5.md`. + UAT r6 (table Object + highlight sanity guards) — Bug 1: table-type field pretty-JSON display + inline edit disabled with note; Bug 2: `evaluateHighlight()` guards (table-type-skip, degenerate, out-of-bounds) + `[HIGHLIGHT_SKIP]` debug logging + ⌗ affordance next to skipped field name; server-side coord fix tracked as new OCR-10. tsc + build clean, v1 md5 unchanged. See `pm/reports/UI-7-uat-r6.md`. + table renderer (v1-style `<table>` display for table-type fields in Fields tab, headers from first row's keys, empty array → "ตาราง (0 rows)" i18n key; drawer JSON view + disabled inline edit preserved from r6). Shipped together with OCR-10 Phase 1 quick win (see OCR-10 row). | — |
...
| OCR-10 | Server-side crop-pass coord audit — highlight offset root cause per UAT r5+r6 red flag (client normalization + sanity guards เป็น band-aid; ต้องหา coord space ที่ server ใช้แล้ว emit fraction consistent) | ocr-pipeline | 🟡 P1 | 🔨 in progress — Phase 1 quick win landed (client-side text-layer bbox validation via Compare's `getPageTextLayer` + `findInTextLayer`; flag `ENABLE_OCR_HIGHLIGHT_VALIDATION = true`; text-based PDFs now snap highlight to text run instead of trusting AI bbox blindly; scan-PDF/image/no-match/table = existing r6 AI-bbox fallback). Server-side coord audit (Phase 2) still pending. See `pm/reports/OCR-10-quickwin.md`. | UI-7 UAT r6 |
...
| OCR-9 | Same-name field on multiple pages returns only page-1 value (UAT 2026-07-15 #7) | ocr-pipeline | 🟡 P1 | 👀 review — Option B (prompt rule, no schema change) landed: SHARED_RULES_VERSION → `2026-07-15-v3` + rule "ถ้า field ปรากฏหลายหน้าและค่าต่างกัน คืน `[หน้า 1: X \| หน้า 2: Y]`"; new benchmark case `multipage-same-field-different-values` (multi-page category, 2-page HTML fixture, DOC-001/DOC-002, assertion=multiline min 0.66); tsc + build clean, v1 OCRWorkspace untouched. Report: `pm/reports/OCR-9.md`. Post-deploy: run new case + full 13-case baseline regression (must not drop from 91.7% case-level). Follow-up OCR-9b (structural multi-value) deferred | — |
