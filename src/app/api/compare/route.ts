...
export async function POST(req: NextRequest) {
    try {
        ...
        const prompt = `เปรียบเทียบรูปภาพเอกสารทั้ง ${files.length} ฉบับนี้และหาจุดที่แตกต่างกันทั้งหมดอย่างละเอียด (ข้อความ ตัวเลข ราคา ที่อยู่ หรือรายละเอียดอื่นๆ)
รูปแบบการตอบกลับต้องเป็น JSON เท่านั้น ดังนี้:
{
    "differences": [
        {
            "description": "คำอธิบายสิ่งที่แตกต่าง (เช่น ชื่อบริษัทเปลี่ยนจาก A เป็น B)",
...
* กฎเหล็กเรื่องพิกัด Bounding Box (สำคัญมาก):
1. ค่าพิกัด bbox ต้องเป็น [ymin, xmin, ymax, xmax] โดยเป็นตัวเลขระหว่าง 0 ถึง 1000 เท่านั้น (ymin=ขอบบนสุด, xmin=ขอบซ้ายสุด, ymax=ขอบล่างสุด, xmax=ขอบขวาสุด)
2. กรอบ bbox ต้องแคบและ "แนบชิด" กับตัวอักษรของข้อความที่แตกต่างให้มากที่สุด ห้ามครอบคลุมพื้นที่ว่างเกินความจำเป็น และต้องระวังตำแหน่งแกน y และ x ให้แม่นยำ
3. หากเป็นการแก้ไขจุดเล็กๆ เช่น ตัวเลข 1 ตัว ให้ตีพิกัดเฉพาะตัวค่านั้นๆ อย่าตีครอบทั้งบรรทัด
4. ระบบพิกัดของแต่ละรูปแยกจากกัน (bbox1 ใช้สัดส่วนของรูปแรก, bbox2 ใช้สัดส่วนของรูปที่สอง)
กรุณาตอบกลับแค่ JSON เพียวๆ ไม่มี markdown หรือ text อื่น และหากไม่มีความแตกต่างให้ส่ง array ว่างกลับมา`;
